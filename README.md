...
- It is O(1). Add/Remove to front and back is O(n) and is O(1) respectively.

* What is the worse case scenario if you try to extend the storage size of a dynamic array?
- Worst case scenario is O(n) where you have to create a copy of the array and copy its old values and append a new value
...
- The new block is inspected to check if it as the correct hashd value. The hash value is the proof + previous_hash where the proof is generated by guess and check.

* Why can't someone cheat by changing a transaction from an earlier block to give themselves coins?
- The longest chain is the most trusted, so changing a transaction means changing the hash which makes all subsequent blocks incorrect.
- and changing them is computationally expensive. 
